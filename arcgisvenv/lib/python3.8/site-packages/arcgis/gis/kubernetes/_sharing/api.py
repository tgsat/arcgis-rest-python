...
class KbertnetesPy(object):
        ...
        **kwargs,
        ...
        if url:
            ...
            if normalized_url.lower().find("www.arcgis.com") > -1:
                ...
                self.resturl = (
                    "{scheme}://www.arcgis.com/sharing/rest/".format(
                        scheme=urlscheme
                    )
                )
    ...
    def get_version(self, force: bool = False):
        ...
        if not self._version or force:
            ...
            if not resp:
                ...
                if resp:
                    _log.warning(
                        "Portal is pre-1.6.2; some things may not work"
                    )
                ...
                if version == "1.6.2" or version == "2.0":
                    _log.warning(
                        "Portal is pre-2.1; some features not supported"
                    )
                    self._is_pre_21 = True
            if resp:
    ...
    def add_group_users(
        ...
        **Parameter**  **Description**
        ...
        if self._is_pre_21:
            _log.warning(
                "The auto_accept option is not supported in "
                + "pre-2.0 portals"
            )
            return
        ...
        resp = self.con.post(
            "community/groups/" + group_id + "/addUsers", postdata
        )
        return resp
    ...
    def create_group_from_dict(
        ...
        **Parameter**  **Description**
        ...
        if thumbnail:
            ...
            files.append(
                ("thumbnail", thumbnail, os.path.basename(thumbnail))
            )
    ...
    def get_group_content(self, group_id: str, max_items: int = 10):
        ...
        content = self.con.post(
            f"{self.resturl}content/groups/{group_id}", params
        )
    ...
    def invite_group_users(
        ...
        **Parameter**      **Description**
        ...
        resp = self.con.post(
            "community/groups/" + group_id + "/invite", postdata
        )
    ...
    def reassign_group(self, group_id: str, target_owner: str):
        ...
        **Parameter**      **Description**
        ...
        resp = self.con.post(
            "community/groups/" + group_id + "/reassign", postdata
        )
        if resp:
            return resp.get("success")
    ...
    def remove_group_users(self, user_names: list[str], group_id: str):
        ...
        **Parameter**      **Description**
        ...
        postdata["users"] = ",".join(user_names)
        resp = self.con.post(
            "community/groups/" + group_id + "/removeUsers", postdata
        )
        return resp
    ...
    def add_item(
        ...
        **Parameter**        **Description**
        ...
        if data:
            if isinstance(
                data, (io.BytesIO, io.StringIO)
            ) == False and _is_http_url(data):
    ...
    def publish_item(
        ...
        if publishParameters is not None and isinstance(
            publishParameters, dict
        ):
    ...
    def create_folder(self, owner: str, title: str):
        ...
        **Parameter**      **Description**
        ...
        resp = self.con.post(
            "content/users/" + owner + "/createFolder", postdata
        )
        if resp and resp.get("success"):
            return resp["folder"]
    ...
    # ----------------------------------------------------------------------
    def move_item(
        self, itemid: str, owner: str, current_folder: str, folder_id: str
    ):
    ...
    # ----------------------------------------------------------------------
    def can_delete(
        self, item_id: str, owner: str, folder: Optional[str] = None
    ):
        ...
        **Parameter**           **Description**
        ...
        if data:
            if isinstance(data, dict):
                ...
                files.append(
                    ("file", data, item_properties.get("fileName", None))
                )
        ...
        if item_properties and item_properties.get("screenshots", None):
            for screenshot in item_properties.get("screenshots", [])[0:4]:
                files.append(
                    ("screenshot", screenshot, os.path.basename(screenshot))
                )
    ...
    def reassign_user(self, username: str, target_username: str):
        ...
        **Parameter**      **Description**
        ...
        resp = self.con.post(
            "community/users/" + username + "/reassign", postdata
        )
        if resp:
            return resp.get("success")
    ...
    def usage(
        ...
        return self.con.post(
            "portals/self/usage", postdata, use_ordered_dict=True
        )
    ...
    def update_user_role(self, username: str, role: str):
        ...
        **Parameter**      **Description**
        ...
        resp = self.con.post(
            "portals/self/updateuserrole", postdata, ssl=True
        )
        if resp:
            return resp.get("success")
    ...
    def is_arcgisonline(self):
        ...
        return (
            self._properties["portalName"] == "ArcGIS Online"
    ...
    # ----------------------------------------------------------------------
    def generate_token(
        self, username: str, password: str, expiration: int = 60
    ):
