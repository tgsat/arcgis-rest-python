...
class UnsupportedWarning(DeprecatedWarning):
    ...
    def __str__(self):
        ...
        return (
            "%(function)s is unsupported as of %(removed)s."
            "%(details)s" % (parts)
        )
...
def deprecated(
    ...
    if deprecated_in is None and removed_in is not None:
        raise TypeError(
            "Cannot set removed_in to a value "
    ...
    def _function_wrapper(function):
        ...
        def _inner(*args, **kwargs):
            if should_warn:
                ...
                warnings.warn(
                    the_warning, category=DeprecationWarning, stacklevel=3
                )
